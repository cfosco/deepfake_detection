...
def test_full_image_network(
    ...
    while reader.isOpened():
        ...
        # Init output writer
        if writer is None and make_video:
...
if __name__ == '__main__':
    ...
    p.add_argument('--test_targets', '-t', type=str, default=None)
    ...
    if args.test_targets is not None:
        with open(args.test_targets) as f:
            targets = json.load(f)
    del args.test_targets
    correct = []
    ...
    if video_path.endswith('.mp4') or video_path.endswith('.avi'):
        ...
        for video in videos:
            ...
            pred = test_full_image_network(**vars(args))
            correct.append(1 if pred == targets[video] else 0)
