...
from dataloader import ImageFromFolder
...
parser.add_argument('--dataset_dir', type=str, default='MotionMagnification/train')
parser.add_argument('--num_resblk_enc', type=int, default=3)
parser.add_argument('--num_resblk_man', type=int, default=1)
parser.add_argument('--num_resblk_dec', type=int, default=9)
...
def main():
    ...
    # create model
    model = MagNet(
        num_resblk_enc=args.num_resblk_enc,
        num_resblk_man=args.num_resblk_man,
        num_resblk_dec=args.num_resblk_dec,
    )
    ...
    dataset_mag = ImageFromFolder(
        os.path.join(DATA_ROOT, args.dataset_dir),
    ...
    optimizer = torch.optim.Adam(
        model.parameters(), args.lr, betas=(0.9, 0.999), weight_decay=args.weight_decay
    )

    # train model
    for epoch in range(args.start_epoch, args.epochs):
        loss_recon, loss_reg1, loss_reg2, loss_reg3 = train(
            data_loader, model, criterion, optimizer, epoch, args
        )
        ...
        # save checkpoints
        save_name = f'ckpt_{args.num_resblk_enc}_{args.num_resblk_man}_{args.num_resblk_dec}_{epoch}.pth.tar'
        fpath = os.path.join(ckpt_dir, save_name)
        ...
        if i % args.print_freq == 0:
            print(
                ...
                'LossR3 {loss_reg3.val:.4f} ({loss_reg3.avg:.4f})'.format(
                    ...
                    data_time=data_time,
                    loss=losses_recon,
                    loss_reg1=losses_reg1,
                    loss_reg2=losses_reg2,
                    loss_reg3=losses_reg3,
                )
            )
