...
from dataloader import ImageFromFolderTest
from network import MagNet
from utils import vidwrite
...
parser.add_argument(
    '--load_ckpt', type=str, metavar='PATH', help='path to load checkpoint'
)
parser.add_argument('--fake_ckpt', type=str, default=None)
parser.add_argument(
    '--save_dir',
    default='test_output',
    type=str,
...
parser.add_argument(
    '-m', '--amp', default=4.0, type=float, help='amplification factor (default: 10.0)'
)
parser.add_argument(
    '--mode',
    default='static',
    type=str,
    choices=['static', 'dynamic', 'temporal'],
    help='amplification mode (static, dynamic, temporal)',
)
parser.add_argument(
    '--video_path', default='test_data/baby', type=str, help='path to video frames'
)
parser.add_argument('--num_data', default=300, type=int, help='number of frames')
# for temporal filter
parser.add_argument('--fh', default=1, type=float)
parser.add_argument('--fl', default=0.5, type=float)
