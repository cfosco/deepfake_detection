...
IMAGE_EXTENSIONS = ['.jpg', '.jpeg', '.png', '.ppm', '.bmp', '.pgm', '.tif', '.JPEG']
...
class ImageFromFolder(ImageFolder):
    ...
    def __getitem__(self, index):
        ...
        sampleAmp, sampleA, sampleB, sampleC = np.array(
            self.loader(pathAmp)), np.array(
            self.loader(pathA)), np.array(
            self.loader(pathB)), np.array(
                self.loader(pathC))
        ...
        # to torch tensor
        sampleAmp, sampleA, sampleB, sampleC = torch.from_numpy(sampleAmp), torch.from_numpy(
            sampleA), torch.from_numpy(sampleB), torch.from_numpy(sampleC)
...
class ImageFromFolderTest(ImageFolder):
    ...
    def __init__(self, root, mag=10.0, mode='static', num_data=300, preprocessing=False,
                 transform=None, target_transform=None, loader=default_loader):

        imfiles = sorted([os.path.join(root, f)
                          for f in os.listdir(root)
                          if any(f.endswith(ext) for ext in IMAGE_EXTENSIONS)])
        if len(imfiles) == 0:
            raise(RuntimeError(f'Found 0 img files in {root}'))
        if mode == 'static' or mode == 'temporal':
            imgs = [(imfiles[0], imfiles[i + 2], mag) for i in range(len(imfiles) - 2)]
            # imgs = [(root + '_%06d.png' % (1),
            #  root + '_%06d.png' % (i + 2),
            #  mag) for i in range(num_data)]
        elif mode == 'dynamic':
            imgs = [(imfiles[i + 1], imfiles[i + 2], mag) for i in range(len(imfiles) - 2)]
