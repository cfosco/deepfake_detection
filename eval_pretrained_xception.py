...
import pretorched
import models
from data import VideoFolder, VideoZipFile

from classification import network
from classification import dataset as dset
...
if KAGGLE:
    ...
    WEIGHT_DIR = os.path.join(dir_path, 'weights')
    # WEIGHT_DIR = 'data'
...
def main(
    ...
    **kwargs,
    ...
    checkpoint_file = 'faceforensics_pretrained_xception_all_c23'
    ...
    # fakenet = network.models.model_selection(modelname='xception', num_out_classes=2)
    sys.path.append('classification')
    model_path = (
        'classification/faceforensics++_models_subset/face_detection/xception/all_c23.p'
    )
    fakenet = torch.load(model_path)
    ...
    fakenet.eval()
    detector = XceptionFrameModel(fakenet, normalize=True)
...
class XceptionNormalize(nn.Module):
    def __init__(
        self,
...
class XceptionFrameModel(torch.nn.Module):
    ...
    def __init__(self, model, consensus_func=torch.mean, normalize=False):
        ...
        self.norm = XceptionNormalize() if normalize else nn.Identity()
...
if __name__ == '__main__':
    if KAGGLE:
        main(TEST_VIDEO_DIR, target_file=TARGET_FILE)
    else:
        args = parse_args()
        main(**vars(args))
